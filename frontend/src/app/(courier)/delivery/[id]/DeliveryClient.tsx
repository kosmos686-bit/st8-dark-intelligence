...
export default function DeliveryClient() {
  ...
  async function transition(status: OrderStatus) {
    ...
    try {
      ...
      setOrder(updated)
      if (status === 'delivered') router.push('/route')
    } finally {
      setWorking(false)
    }
  }
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="rounded-xl p-5 mb-4" style={{ background: '#111827', border: '1px solid #1E2A3A' }}>
        ...
        <div className="flex items-center gap-2 text-sm text-slate-400">
          <span>{order.items.length} поз.</span><span>·</span><span>{order.total_amount} ₽</span>
        </div>
        {order.notes && (
      ...
      <div className="space-y-2">
        {order.status === 'assembled' && (
          <button onClick={() => transition('picked_up')} disabled={working}
            className="w-full py-3 rounded-xl font-bold" style={{ background: '#06B6D4', color: 'white' }}>
            Забрал заказ
          </button>
        )}
        {order.status === 'picked_up' && (
          <button onClick={() => transition('delivering')} disabled={working}
            className="w-full py-3 rounded-xl font-bold" style={{ background: '#06B6D4', color: 'white' }}>
            Еду к клиенту
          </button>
        )}
        {order.status === 'delivering' && (
          <button onClick={() => transition('delivered')} disabled={working}
